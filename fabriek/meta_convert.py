...
for mod in models:
    ...
    for st in range(0,48,3):
        ...
        if prof:
            ...
            cmd = 'python /usr/people/plas/python/tools/use_ncprof_cli.py {model} {startdate} {indir} {outdir}'.format(model = mod, startdate=starttime.strftime('%Y%m%d%H'),indir = sdir, outdir = os.path.join(outpath,suites[mod]['shortname'])) 
            pid = subprocess.Popen(cmd,shell=True)
            pid.communicate()
