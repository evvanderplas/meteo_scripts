...
def finddate(somefname):
    ...
    try:
        ...
        leadtime = dtl[4]
        startdate = datetime(year,month,day,hour,0,0) 
        validdate = datetime(year,month,day,hour,0,0) + timedelta(hours=int(leadtime))
...
for infile in args.infiles:
    ...
    if args.date_from_filename:
        ...
        sdtg  = datetime.strptime(str(grb.dataDate),'%Y%m%d') # yyyymmdd
        shour = grb.dataTime/100
        startdate = sdtg + datetime.timedelta(hours=shour)
        ...
        print list(vtime[...])
        print 'Check dtime, time',len(dtime),datetime( year, month, day, hour, minute, seconds ); #continue
        validdate =  datetime( year, month, day, hour, minute, seconds )
        print 'Check resulting datetime',

    print '*********'
    print startdate,validdate
    print 'validdate, seconds (date2num):',date2num(validdate, vtime.units, calendar='standard'), int(date2num(validdate, vtime.units, calendar='standard'))
    print 'validdate, seconds (mktime)', time.mktime(validdate.timetuple()),int(time.mktime(validdate.timetuple()))
    print '*********'
    ...
    if args.verbose:
        #print "Parsing file '%s' for date %s, index %4i, value %i" % (infile, datetime( year, month, day, hour, minute ), windex, writetime )
