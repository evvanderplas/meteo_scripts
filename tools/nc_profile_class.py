...
def loop_over_grib(gfile,info,stationlist,ncdict):

    model,version = info['model'],info['version']
    ...
    for grb in grbs:
        ...
        for s in stationlist.keys():
            ...
            if not ncperdate[s].values.has_key(parindex):
                if levelctype == 109:
                    ncperdate[s].values[parindex] = np.zeros(ncperdate[s].ltmax,ncperdate[s].nlevs+1)
                else:
                    ncperdate[s].values[parindex] = np.zeros(ncperdate[s].ltmax)

                if levelctype == 109:
                    ncperdate[s].values[parindex][lt,level] = iv
                else:
                    ncperdate[s].values[parindex][lt] = iv
...
class ncprofile:
    ...
    def initfile(self,startdate=None,outfile=None):
        ...
        rtime = self.nc.createVariable('forecast_reference_time','i4',())
    ...
    def create1D(self,shortname,longname,units,standardname):
        ...
        vgrib = self.nc.createVariable(shortname,'f4',('time',),
                               zlib=True,
    ...
    def create2D(self,shortname,longname,units,standardname):
        ...
        vgrib = self.nc.createVariable(shortname,'f4',('time','level'),
                               zlib=True,
                               fill_value=netCDF4.default_fillvals['f4'])
    ...
    def create_pl(self,shortname,longname,units,standardname):
        ...
        vgrib = self.nc.createVariable(shortname,'f4',('time','plevel'),
                               zlib=True,
                               fill_value=netCDF4.default_fillvals['f4'])
    ...
    def postproc_radiation(self):
        ...
        lwd = self.nc.variables['lwsd'] #['lwdrad']
        swd = self.nc.variables['swsd'] #['swdrad']
        lwn = self.nc.variables['lwsn'] #['lwnrad']
        swn = self.nc.variables['swsn'] #['swnrad']
        ...
        self.nc.variables['lwsd'][:] = deacc(lwd)
        self.nc.variables['swsd'][:] = deacc(swd)
        self.nc.variables['lwsn'][:] = deacc(lwn)
        self.nc.variables['swsn'][:] = deacc(swn)

        swu = self.create1D('swsu', 'Surface SW up radiation', 'W m**-2','surface_net_upward_shortwave_flux')
        lwu = self.create1D('lwsu', 'Surface LW up radiation', 'W m**-2','surface_net_upward_longwave_flux')
        netrad = self.create1D('radsd','Surface net total radiation', 'W m**-2','surface_net_downward_radiative_flux')
        self.nc.variables['swsu'][:]  = deacc(swd[:]-swn[:]) 
        self.nc.variables['lwsu'][:]  = deacc(lwd[:]-lwn[:]) 
        self.nc.variables['radsd'][:] = deacc(lwn[:]+swn[:]) 
    ...
    def postproc_uv(self):
        ...
        u10   = self.nc.variables['u10m']
        v10   = self.nc.variables['v10m']
        ugust = self.nc.variables['u10mg']
        vgust = self.nc.variables['v10mg']
    ...
    def postproc_pz(self):
        ...
        p_surf  = self.nc.variables['Psurf'][:]
        ...
        self.nc.variables['p_ml'][:] = p_full

        ## calculate thickness/height of levels: 
        #t       = self.nc.variables['T_ml'][0,:,:]
        #q       = self.nc.variables['q_ml'][0,:,:]
        t       = self.nc.variables['T_ml'][:]
        q       = self.nc.variables['q_ml'][:]
        tv      = t + (1 + 0.61 * q)   # virtual temperature
        try:
            oro     = self.nc.variables['z_oro'][:]
        except:
            print 'No orography in netCDF'
            self.create1D('z_oro','Orography','m','height')
            self.nc.variables['z_oro'][:] = np.zeros(self.steps)
            oro = np.zeros(self.steps)
        #ta      = tv[:,1:] + tv[:,:-1]
