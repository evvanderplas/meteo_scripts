...
class ncconv:
    ...
    def initprofiles(self,startdate=None,outfile=None):
        ...
        for s in self.stationlist.keys():
            ...
            slevel   = self.files[s].createDimension('station', None) #self.stationid)
            vstation = self.files[s].createVariable('station','i4',('station',)) 
            vstation[:] = self.stationlist[s]['sid']
        ...
        for s in self.stationlist.keys():
            for shortname in self.values[s].keys():
                ...
                if vals.shape == (self.info['ltmax']+1,):
                    self.create1D(self.files[s],shortname,vals)
                elif vals.shape == (self.info['ltmax']+1,self.nlevs):
                    self.create2D(self.files[s],shortname,vals)
                elif vals.shape == (self.info['ltmax']+1,len(self.pressure_levs)):
                    self.create_pl(self.files[s],shortname,vals)
                else:
                    print 'Huh?!@%',shortname,vals.shape
                    sys.exit(1)

    def create1D(self,ncfile,shortname,values):
        
        #vgrib = self.nc.createVariable(shortname,'f4',('station_id','time',),
        vgrib = ncfile.createVariable(shortname,'f4',('time',),
                                      zlib=True,
    ...
    def create2D(self,ncfile,shortname,values): #shortname,longname,units,standardname):
        
        #vgrib = self.nc.createVariable(shortname,'f4',('station_id','time','level'),
        #vgrib = ncfile.createVariable(shortname,'f4',('time','level'), 
        vgrib = ncfile.createVariable(shortname,'f4',('time','nlev'),
                                     zlib=True,
                                      fill_value=netCDF4.default_fillvals['f4'])
    ...
    def create_pl(self,ncfile,shortname,values): #shortname,longname,units,standardname):
        
        #vgrib = self.nc.createVariable(shortname,'f4',('station_id','time','plevel'),
        vgrib = ncfile.createVariable(shortname,'f4',('time','plevel'),
                                      zlib=True,
                                      fill_value=netCDF4.default_fillvals['f4'])
        ...
        for s in self.stationlist.keys():
            ...
            for var in ['latf','senf','ustress','vstress','lhevap','waterevap']:
                try:
                    ...
                    self.files[s].variables[accdict[var]][:] =  deacc( self.files[s].variables[accdict[var]])
                    self.files[s].variables[accdict[var]].long_name = self.names[accdict[var]][0]
                except KeyError:
                    print 'No deaccumulation: not in parsed grib: ',var

            lwd = self.files[s].variables[accdict['lwsd']] #['lwdrad']
            swd = self.files[s].variables[accdict['swsd']] #['swdrad']
            lwn = self.files[s].variables[accdict['lwsn']] #['lwnrad']
            swn = self.files[s].variables[accdict['swsn']] #['swnrad']

            swnet,lwnet   =  deacc(swn),deacc(lwn)
            swdown,lwdown =  deacc(swd),deacc(lwd)
 
            self.files[s].variables[accdict['swsd']][:] = swdown
            self.files[s].variables[accdict['lwsd']][:] = lwdown
            self.create1D(self.files[s],'SWnet',swnet)
            self.create1D(self.files[s],'LWnet',lwnet)

            self.create1D(self.files[s],'swsu' ,swdown - swnet) #deacc(swd[:]-swn[:]))
            self.create1D(self.files[s],'lwsu' ,lwdown - lwnet) #deacc(lwd[:]-lwn[:]))
            self.create1D(self.files[s],'radsd',lwnet + swnet ) #deacc(lwn[:]+swn[:]))

            if 1: #for var in ['latf','senf','ustress','vstress','lhevap','waterevap']:
                try:
                    self.create1D(self.files[s],'lhflux',deacc(self.files[s].variables[accdict['lhevap']][:]))
    ...
    def postproc_uv(self):
        ...
        for s in self.stationlist.keys():
            ...
            if 0: # naming
                ...
                self.create1D(self.files[s],'Vamp10m', towindspeed(u10[:],v10[:]))
                self.create1D(self.files[s],'Vdir10m', towinddir(u10[:],v10[:])  )
                self.create2D(self.files[s],'Vamp',    towindspeed(u_ml[:],v_ml[:])  )
                self.create2D(self.files[s],'Vdir',    towinddir(u_ml[:],v_ml[:])   )

            self.create1D(self.files[s],'gust',     towindspeed(ugust[:],vgust[:]))
            self.create1D(self.files[s],'gustdir',  towinddir(ugust[:],vgust[:]  ))
    ...
    def postproc_pz(self):
        ...
        for s in self.stationlist.keys():
            ...
            p_surf  = self.files[s].variables['Psurf'][:]
            ...
            p    = self.create2D(self.files[s],'p',p_full)

            ## calculate thickness/height of levels: 
            t       = self.files[s].variables['t'][:]
            q       = self.files[s].variables['q'][:]
            tv      = t + (1 + 0.61 * q)   # virtual temperature
            try:
                oro     = self.files[s].variables['z_oro'][:]
            except:
                ...
                self.create1D(self.files[s],'z_oro',oro)
            ...
            self.create2D(self.files[s],'height_f', z_mid)
	...
	rh = q.sdata/qsat 
        ...
        for s in self.stationlist.keys():
            ...
            t       = self.files[s].variables['t'][:]
            q       = self.files[s].variables['q'][:]
            p       = self.files[s].variables['p'][:] # created in postproc_pz

            rh,td   = calc_td(t,q,p)

            self.create2D(self.files[s],'td', td + t_0)
            self.create2D(self.files[s],'rh',rh)
